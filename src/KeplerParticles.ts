...
export class KeplerParticles {
    ...
    constructor(options: KeplerParticlesOptions, contextOrSimulation: Simulation) {
        ...
        const defaultMapTexture = getThreeJsTexture(this.options.textureUrl, this.context.options.basePath);

        this.uniforms = {
            texture: { value: defaultMapTexture },
        };

        const particleCount = this.options.maxNumParticles || DEFAULT_PARTICLE_COUNT;
        this.elements = [];
        this.attributes = {
        ...
        Object.keys(this.attributes).forEach((attributeName) => {
            const attribute = this.attributes[attributeName as keyof ShaderAttributes];
